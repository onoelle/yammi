...
void YammiModel::readPreferences()
{
	...
	config.pmBaseDir=d.absPath();
	QDomDocument doc( "prefs" );
	QFile f( config.pmBaseDir+"/prefs.xml" );
	...
	config.criticalSize			=	getProperty(&doc, "criticalSize", 700);
	cout << "..done\n";
}
...
void YammiModel::savePreferences()
{
	...
	QFile f2( config.pmBaseDir+"/prefs.xml" );
...
bool YammiModel::startFirstTime()
{
	...
	QDir d = QDir::home();  // now points to home directory
 	...
 	d.cd(".yammi");
 	cout << " ..done\n";
 	cout << "creating subdirectory playlists to store your playlists...";
 	if ( !d.mkdir( "playlists" ) ) {
 		cout << "\nERROR: Could not create directory playlists...";
 		return false;
 	}
 	cout << " ..done\n";
	...
	// touch history file ???
//	QFile f(config.pmBaseDir+"/logfile.log");
//	if ( !f.open( IO_ReadOnly ) ) {
...
/**
 * Reads the playlist from the found xml files in playlist directory.
 */
void YammiModel::readPlayLists()
{
	// read in all xml-files found in a given directory that represent a playlist
	cout << "reading playlists...\n";
	
	QDir d(config.pmBaseDir+"/playlists");
	...
	for ( ;(fi=it.current()); ++it ) {           // for each file
		...
		if(docElem.tagName()!="playlist") continue;
		QDomNode n = docElem.firstChild();
   	QString playlistName(docElem.attribute("name"));
		
		// add playlist to database
		MyList* ptr=new MyList;
		allPlaylists.append(ptr);
		...
		while( !n.isNull() ) {
			...
			if( !e.isNull() ) { 											// the node is really an element.
				...
				if(!found) {
					cout << "Playlist item " << artist << " - " << title << " not found in song database\n";
...
/// save playlists (if changed) to xml-files
void YammiModel::savePlayLists()
{
	// save all playlists to xml-files
	cout << "saving playlists...\n";
	
	// for all playlists...
	QString playlistName=playlistNames.first();
	for(MyList* ptr=allPlaylists.first(); ptr; ptr=allPlaylists.next()) {
		// create xml-file
		QDomDocument doc( "playlist" );
		QString empty("<?xml version=\"1.0\" encoding=\"UTF-16\"?><!DOCTYPE songdb>\n<playlist>\n</playlist>\n");
		if ( !doc.setContent( empty ) ) {
			cout << "ERROR: could not save playlists!\n";
			return;
		}
		
		QDomElement rootElem = doc.documentElement();
		rootElem.setAttribute("name", playlistName);
		...
		QString save=doc.toString();
		QString playlistFilename=QString("%1.xml").arg(playlistName);
		QFile f2( config.pmBaseDir+"/playlists/"+playlistFilename );
		if ( !f2.open( IO_WriteOnly  ) )
			return;
		f2.writeBlock ( save, save.length() );
		f2.close();
		playlistName=playlistNames.next();
	}
	cout << "..done\n";
...
void YammiModel::readSongDatabase()
{
	...
	QFile f( config.pmBaseDir+"/songdb.xml" );
	if ( !f.open( IO_ReadOnly ) ) {
...
void YammiModel::saveSongDatabase()
{
	...
	system("mv "+config.pmBaseDir+"/songdb.xml "+config.pmBaseDir+"/songdb_backup.xml" );
	QString save=doc.toString();
	QFile f2( config.pmBaseDir+"/songdb.xml");
	if(!f2.open(IO_WriteOnly)) {
...
//mmm	tbSaveDatabase->setEnabled(_allSongsChanged || _playlistsChanged);
}
...
void YammiModel::playlistsChanged(bool changed)
{
	_playlistsChanged=changed;
//mmm	tbSaveDatabase->setEnabled(_allSongsChanged || _playlistsChanged);
}

bool YammiModel::playlistsChanged()
{
	return _playlistsChanged;
}
...
//mmm			forSong(newSong, DeleteFile);
			continue;
		}
...
void YammiModel::removePlaylist(QString playlistName)
{
	QString name=playlistNames.first();
	for(MyList* ptr=allPlaylists.first(); ptr; ptr=allPlaylists.next(), name=playlistNames.next()) {
		if(name==playlistName) {
			cout << "removing item: " << name << "\n";
			allPlaylists.remove();
			playlistNames.remove();
			QDir d("/");
			QString todel=QString("%1/playlists/%2.xml").arg(config.pmBaseDir).arg(playlistName);
			d.remove(todel);
...
void YammiModel::newPlaylist(QString playlistName)
{
	MyList* newList=new MyList;
	allPlaylists.append(newList);
	playlistNames.append(playlistName);
	playlistsChanged(true);
}
...
/**
 * saves changed information (playlists + songDatabase)
 */
void YammiModel::save()
{
	if(playlistsChanged() || allSongsChanged())
		savePlayLists();
	if(allSongsChanged())
		saveSongDatabase();
}
